...
begin
  if not exists (
    ...
    where schemaname = 'storage'
      ...
      and policyname = 'Public read inventory media'
  ) then
    create policy "Public read inventory media"
    on storage.objects
    for select
    to public
  ...
  if not exists (
    ...
    where schemaname = 'storage'
      and tablename = 'objects'
      and policyname = 'Public upload inventory media'
  ) then
    create policy "Public upload inventory media"
    on storage.objects
    for insert
    to public
  ...
  if not exists (
    ...
    where schemaname = 'storage'
      and tablename = 'objects'
      and policyname = 'Public update inventory media'
  ) then
    create policy "Public update inventory media"
    on storage.objects
    for update
    to public
    using (bucket_id = 'inventory-media')
  ...
  if not exists (
    ...
    where schemaname = 'storage'
      and tablename = 'objects'
      and policyname = 'Public delete inventory media'
  ) then
    create policy "Public delete inventory media"
    on storage.objects
    for delete
    to public
    using (bucket_id = 'inventory-media');
...
drop function if exists public.create_location(text, text, uuid, text, text);
...
begin
  ...
  insert into public.locations (name, type, parent_id, description, icon_name, image_path)
  values (
    trim(loc_name),
...
drop function if exists public.update_location(uuid, text, text, uuid, text, text);
...
drop function if exists public.create_item(text, uuid, text);
...
begin
  ...
  insert into public.items (name, location_id, icon_name, image_path)
  values (
    trim(item_name),
    target_location,
...
drop function if exists public.update_item_details(uuid, text, text, numeric, date, text, text);
...
grant all on function public.create_location(text, text, uuid, text, text, text) to anon;
grant all on function public.create_location(text, text, uuid, text, text, text) to authenticated;
grant all on function public.create_location(text, text, uuid, text, text, text) to service_role;

grant all on function public.update_location(uuid, text, text, uuid, text, text, text) to anon;
grant all on function public.update_location(uuid, text, text, uuid, text, text, text) to authenticated;
grant all on function public.update_location(uuid, text, text, uuid, text, text, text) to service_role;

grant all on function public.create_item(text, uuid, text, text) to anon;
grant all on function public.create_item(text, uuid, text, text) to authenticated;
grant all on function public.create_item(text, uuid, text, text) to service_role;

grant all on function public.update_item_details(uuid, text, text, numeric, date, text, text, text) to anon;
grant all on function public.update_item_details(uuid, text, text, numeric, date, text, text, text) to authenticated;
grant all on function public.update_item_details(uuid, text, text, numeric, date, text, text, text) to service_role;
